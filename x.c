...
static void alpha(const Arg *);
static void alphaabs(const Arg *);
static void alphareset(const Arg *);
...
alpha(const Arg *arg)
{
	...
	larg.f = bgalpha + arg->f;
	alphaabs(&larg);
}

void
alphaabs(const Arg *arg)
{
	bgalpha = arg->f;
	LIMIT(bgalpha, 0.0, 1.0);
	dc.col[defaultbg].color.alpha = (unsigned short)(0xffff * bgalpha);
	dc.col[defaultbg].pixel &= 0x00FFFFFF;
	dc.col[defaultbg].pixel |= (unsigned char)(0xff * bgalpha) << 24;
	cresize(0, 0);
	redraw();
	xhints();
}
...
main(int argc, char *argv[])
{
	...
	case 'a':
		...
		break;
	case 'A':
		bgalpha = strtof(EARGF(usage()), NULL);
		LIMIT(bgalpha, 0.0, 1.0);
		break;
	case 'c':
